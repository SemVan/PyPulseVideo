...
from signal_fragment_filtration import check_if_fragment_is_reliable
...
def RR_procedure(full_signal):
    window = 131
    rrs = []
    start_point = 0
    while start_point < len(full_signal) - window:
        signal = full_signal[start_point : start_point+window]
        if check_if_fragment_is_reliable(signal):
